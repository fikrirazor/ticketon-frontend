...
const TransactionStatus: React.FC<TransactionStatusProps> = ({
  ...
  return (
    <div className={`space-y-8 ${className}`}>
      ...
      <div className={`px-6 py-4 rounded-2xl border-2 flex items-center justify-between ${getStatusColor(status)}`}>
        <div>
          <span className="text-xs font-black uppercase tracking-widest opacity-70">Order ID: {transactionId}</span>
          <h3 className="text-xl font-black uppercase tracking-tight">{getStatusLabel(status)}</h3>
        </div>
            ...
            return (
              <div key={step.key} className="flex flex-col items-center flex-1">
                ...
                <div className="mt-4 text-center">
                  <p className={`text-sm font-black uppercase tracking-tight ${isActive || isCompleted ? 'text-slate-900' : 'text-slate-400'}`}>
                    {step.label}
                  </p>
      ...
      {['REJECTED', 'EXPIRED', 'CANCELED'].includes(status) && (
        <div className="bg-slate-50 p-6 rounded-2xl text-center border border-slate-100">
          <p className="text-slate-600 font-medium">
            {status === 'REJECTED' && "Your payment proof was rejected by the admin. Please contact support."}
            {status === 'EXPIRED' && "The transaction timer has expired. Please create a new booking."}
      ...
      <div className="flex flex-col md:flex-row gap-4 pt-4">
        ...
        {status === 'DONE' && (
          <Button className="flex-1" variant="success">Download E-Tickets</Button>
        )}
        {(status === 'REJECTED' || status === 'EXPIRED') && (
          <Button className="flex-1" onClick={() => onAction?.('RETRY')}>Try Again</Button>
        )}
