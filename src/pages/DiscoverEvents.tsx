          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 space-y-12">
            <div className="text-center space-y-6">
              ...
              <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-tight">
                Temukan <span className="text-primary italic">Pengalaman</span>{" "}
                Barumu
              </h1>
            </div>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                <div
                  key={i}
                  className="bg-slate-100 rounded-3xl h-96 animate-pulse"
                />
              ))}
            </div>
          ) : events.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center bg-slate-50 rounded-[4rem] border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-500 font-medium max-w-sm mx-auto">
                Coba ubah kata kunci pencarianmu atau atur ulang filter
                lokasinya ya!
              </p>
            </div>
          )}
